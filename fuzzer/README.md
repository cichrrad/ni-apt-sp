...
I've made sure that the legacy (task 1 and 2) test suites still work + added tests for the new parts (`lib/greybox`).
...
* **ISSUE? (SHOULD BE RESOLVED I HOPE) -- Sometimes, statistics dont save, even though I kill the campaign 10 seconds earlier** -- I think this is no longer an issue as I introduced periodic saving AND reordered `stop` method so that saving now takes place before waiting for workers to die, which I think might have been the root cause ?